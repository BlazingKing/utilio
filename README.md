...
- **PWA** — ติดตั้งลงหน้าจอมือถือ/เดสก์ท็อปได้ และ **ใช้งานออฟไลน์จริง** ผ่าน service worker
  (หน้าที่เคยเปิดจะใช้ได้แม้ไม่มีเน็ต)
- **SEO** — `sitemap.xml`, `robots.txt` และ OG image สร้างอัตโนมัติจาก registry
...
> **สำคัญ:** ตั้ง environment variable `NEXT_PUBLIC_SITE_URL` ให้เป็นโดเมนจริง (เช่น `https://utilio.app`)
> เพื่อให้ `sitemap.xml`, `robots.txt` และ OG image ชี้ URL ถูกต้อง — ถ้าไม่ตั้งจะใช้ค่าเริ่มต้น
> `https://utilio.vercel.app`
